...
本章节介绍 Cobalt Skymap 的数据管理系统，涵盖前后端数据流、持久化策略和存储架构。

## 概览

Cobalt Skymap 使用统一的 JSON 文件存储系统，所有用户数据存储在平台特定的应用数据目录中。数据模块位于 `src-tauri/src/data/`，提供设备管理、位置管理、目标列表、标记和观测日志等功能。前端通过 `TauriSyncProvider` 组件与 Rust 后端同步状态。
